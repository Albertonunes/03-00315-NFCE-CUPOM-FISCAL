...
type
  TFRM_PGTOPEDCLI = class(TForm)
    ...
    ck_baixarfinanceiro: TcxCheckBox;
...
begin
  ...
  Begin
    ...
    try
      ...
      MDS := 'NFCE' ;
      ACT := 'FINALIZAR ERRO'   ;
      OBS := 'PED'+FormatFloat('#####',CXPEDIDO.Value) + '/' +' F-'+IntToStr(PRO_FILIAL) + ' C-'+IntToStr(CLIENTE);
...
begin
  ...
  Atualiza_Calculo.Execute;
  cxc_fpgto.SetFocus;
  ck_baixarfinanceiro.Checked := true;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      //QryInsereReceberESPDOC.Value            := QryCstPedESPDOC.Value;
      ...
      QryInsereReceberSALDODOCUMENTO.Value    := iif(ck_baixarfinanceiro.Checked,0,RxPgtoFinalVALOR_REAL.Value);
      ...
      QryInsereReceberPOSICAO.Value           := iif(ck_baixarfinanceiro.Checked,'BAIXADO','ABERTA');
      QryInsereReceberOBSATRASO.Value         := RxPgtoFinalFORMA.Value;
      QryInsereReceberPEDID.Value             := PEDID;
      // fazer a baixa do documento no financeiro e movimentar corretamente a baixa (dinheiro/cartão)
      ...
      try
        ...
        MDS := 'NFCE' ;
        ACT := 'FECHAMENTO ERRO'   ;
        OBS := 'PED'+FormatFloat('#####',CXPEDIDO.Value) + '/' + INTTOSTR(I)+' F-'+IntToStr(PRO_FILIAL) + ' C-'+IntToStr(CLIENTE);
        DmdPrincipal.actLogGeral.Execute ;
      end;
      RxPgtoFinal.Next;
    end;
